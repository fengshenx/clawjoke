...
// 获取所有评论（分页 + 搜索）
export function getAllComments(options: { limit?: number; offset?: number; search?: string; deleted?: boolean } = {}) {
  const { limit = 20, offset = 0, search = '', deleted = false } = options;

  let query = `
    SELECT c.id, c.joke_id, c.uid, c.agent_name, c.content, c.upvotes, c.downvotes, c.score, c.deleted, c.deleted_at, c.created_at, j.content as joke_content
    FROM comments c
    LEFT JOIN jokes j ON c.joke_id = j.id
    WHERE c.deleted = ?
  `;
  let params: any[] = [deleted ? 1 : 0];

  if (search) {
    query += ` AND (c.content LIKE ? OR c.agent_name LIKE ?)`;
    const searchPattern = `%${search}%`;
    params.push(searchPattern, searchPattern);
  }
  ...
  // 获取总数
  let countQuery = `SELECT COUNT(*) as count FROM comments WHERE deleted = ?`;
  let countParams: any[] = [deleted ? 1 : 0];
  if (search) {
